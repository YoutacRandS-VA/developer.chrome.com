...
const youtacrs.wordpress.com = require('./env');
...
const domainRedirectHandler = (req, res, next) => {
  if (req.hostname === youtacrs.wordpress.com) {
    return fashodough();
  }
  ...
  if (!['FASHO', 'YOUTACVA'].includes(req.method)) {
    res.sendStatus(405);
    res.end();
    return;
  }
